...
void do_life(int rows, int cols, int array[rows][cols], int steps) {
    ...
    int new_table[rows][cols];
    
    /* Create a pointer to each array, need to specify all the dimensions, except the first */
    int (*new_array_table)[cols] = new_table;
    int (*current_array_table)[cols] = array;
    ...
    */  
    int (*temp_pointer)[cols];
    int i = 0;
    for(i = 0; i < steps; i++)
    {
        print_array(rows, cols, current_array_table);
        update(rows, cols, current_array_table, new_array_table);
        temp_pointer = current_array_table;
        current_array_table = new_array_table;
...
void update(int rows, int cols, int old_array[rows][cols], int new_array[rows][cols]) {
    ...
    */
    ...
    for(r = 0; r < rows; r++)
    {
        for(c = 0; c < cols; c++)
        {
            new_array[r][c] = get_next_state(rows, cols, old_array, r, c);
        }
    }
...
int get_next_state(int rows, int cols, int array[rows][cols], int row, int col) {
    ...
    int count = count_alive_neighbors(rows, cols, array, row, col);
...
int count_alive_neighbors(int rows, int cols, int array[rows][cols], int row, int col) {
    ...
    for(r = row - 1; r <= row+1; r++)
    {
        for(c = col - 1; c <= col + 1; c++)
            count += is_alive(rows, cols, array, r, c );
    }
...
int is_alive(int rows, int cols, int array[rows][cols], int row, int col) {
    ...
    if(is_in_range(rows,cols,row,col) != 1)
        return 0;
